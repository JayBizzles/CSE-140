...
void Decode ( unsigned int instr, DecodedInstr* d, RegVals* rVals) {
    d->op = instr >> 26; // gets the opcode of the bits and stores in op
    if(op == 0){    //checks the value of opcode to see if this is R type
        d->type = R; 
        ...
        if(d->op == 0x2 || d-> == 0x3){
            ...
            d->regs.i.add_or_immed = (instr<<16)>>16;
            // move the values to rvals
